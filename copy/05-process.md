# Sekce 5: JAK TO FUNGUJE — 4 kroky

---

## Nadpis sekce

```
Od první konzultace k čtvrtletním výplatám ve 4 krocích
```

---

## Krok 1

```
┌─────────────────────────────────────────┐
│  KROK 1                                 │
│  Úvodní konzultace (zdarma)             │
│                                         │
│  Společně probereme Vaše investiční     │
│  cíle, časový horizont a objem          │
│  prostředků. Vysvětlíme model,          │
│  zajištění a rizika. Bez závazků.       │
│                                         │
│  ⏱️ 30 minut · osobně nebo online       │
└─────────────────────────────────────────┘
```

---

## Krok 2

```
┌─────────────────────────────────────────┐
│  KROK 2                                 │
│  Výběr investiční příležitosti          │
│                                         │
│  Představíme Vám konkrétní projekt:     │
│  nemovitost, lokalitu, ocenění,         │
│  LTV, očekávaný výnos a délku.          │
│  Vy se rozhodnete, zda investovat.      │
│                                         │
│  📊 Kompletní dokumentace k projektu    │
└─────────────────────────────────────────┘
```

---

## Krok 3

```
┌─────────────────────────────────────────┐
│  KROK 3                                 │
│  Smluvní zajištění + převod prostředků  │
│                                         │
│  Podepíšete investiční smlouvu          │
│  s jasně definovanými podmínkami:        │
│  výnos, termíny výplat, zajištění,      │
│  podmínky ukončení.                     │
│                                         │
│  📝 Smlouva připravena naším právníkem  │
└─────────────────────────────────────────┘
```

---

## Krok 4

```
┌─────────────────────────────────────────┐
│  KROK 4                                 │
│  Čtvrtletní výplaty na Váš účet        │
│                                         │
│  Každé čtvrtletí obdržíte výnos         │
│  přímo na účet:                         │
│                                         │
│  · Březen · Červen · Září · Prosinec    │
│  · Automaticky do 5. pracovního dne     │
│                                         │
│  Dostáváte čtvrtletní report:           │
│  stav nemovitosti, LTV, výkon portfolia.│
│                                         │
│  Po skončení doby investice (1–3 roky)  │
│  se Vám vrací celá jistina.            │
│                                         │
│  💰 Pravidelný příjem bez Vaší práce    │
└─────────────────────────────────────────┘
```

---

## Pod kroky — CTA

```
Chcete vědět, kolik můžete vydělat?

    [ Zobrazit kalkulaci → ]

nebo

    [ Chci konzultaci s investičním týmem → ]
    📞 [telefonní číslo] — volejte v pracovní době
```

---

## Příklad výpočtu (pod kroky, volitelně)

```
PŘÍKLAD: Investice 5 000 000 Kč na 2 roky

Cílový výnos:          10 % p.a.
Roční výnos:           500 000 Kč
Čtvrtletní výplata:    125 000 Kč

Za 2 roky celkem:
  · 8 čtvrtletních výplat:  1 000 000 Kč
  · Návrat jistiny:         5 000 000 Kč
  · Celkem obdržíte:        6 000 000 Kč

⚠️ Jedná se o ilustrativní příklad s cílovým výnosem.
Skutečný výnos se může lišit.
```
